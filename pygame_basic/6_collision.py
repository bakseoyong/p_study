...
while running:
    ...
    # ==> fps 에 따라 동작하는 횟수는 다른데 KEYDOWN이벤트로 이동하는 좌표값은 동일하므로 속도의 차이가 발생하게 된다! => 보정을 해줘야 한다.
    ...
    character_x_pos += to_x * dt # dt를 곱함으로써 프레임이 다르더라도 속도를 보정.
    character_y_pos += to_y * dt

    # 가로 경계값 처리
    if character_x_pos < 0: # 화면 왼쪽보다 이전으로 나가면
        character_x_pos = 0
